 ...
 * Unity project. The project check is memoized per repository path so the
 * common case (a non-Unity repo, or repeated diffs in the same repo) costs at
 * most one filesystem probe.
 *
...
const unityProjectCache = new Map<string, Promise<boolean>>()
...
const isUnityRepository = (repository: Repository): Promise<boolean> => {
  ...
  const probe = isUnityProject(repository.path)
  unityProjectCache.set(repository.path, probe)
  return probe
}
