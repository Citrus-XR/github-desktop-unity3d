/**
 * Mapping from Unity's runtime class IDs (the `!u!<classId>` in document
 * headers) to friendly type names, for Unity 2022.3. Unknown IDs are never
 * discarded — `getClassName` returns a `Class<id>` placeholder so the document
...
/** The built-in class-ID → name table for Unity 2022.3. */
export const unity2022_3ClassNames: ReadonlyMap<number, string> = new Map([
  [0, 'Object'],
  [1, 'GameObject'],
  [2, 'Component'],
  [3, 'LevelGameManager'],
...
export const getClassName = (classId: number): string =>
  classNames.get(classId) ?? `Class${classId}`

/** Transform-like components that participate in the hierarchy tree. */
export const transformClassIds: ReadonlySet<number> = new Set([4, 224])
